 ...
 */
public class RestaurantResourceFilter implements Filter {

    /**
     * A LOGGER available for all the subclasses.
     */
    protected static final Logger LOGGER = LogManager.getLogger(RestaurantResourceFilter.class, StringFormatterMessageFactory.INSTANCE);
    ...
    public void doFilter(final ServletRequest servletRequest, final ServletResponse servletResponse, final FilterChain chain) throws
        ...
        try {
            // get id of the restaurant to be modified from the URI
            String pathinfo = req.getPathInfo();
            String[] tokens = pathinfo.split("/");
            int restaurant_id  = Integer.parseInt(tokens[3]);

            // pass restaurant_id to the request
            req.setAttribute("restaurant_id", restaurant_id);

            if (restaurant_id == 1)
                throw new IOException("Unable to modify this restaurant.");

            // search for the real manager of restaurant_id
            User manager = new GetManagerDAO(ds.getConnection(), restaurant_id).access().getOutputParam();
            ...
            LOGGER.error("Unable to retrieve the manager of the restaurant.", e);
            Message m = new Message("Unable to retrieve the manager of the restaurant.", ErrorCodes.INVALID_INPUT_PARAMETER, e.getMessage());
     ...
     * @param manager manager of the restaurant the client is trying to modify
     *
    ...
    private boolean authenticateUser(HttpServletRequest req, HttpServletResponse res, User manager) {
        ...
        try {
            ...
            if(auth_user != null) {
                ...
                // true if the authenticated user is the manager of the restaurant in the request body
                if(auth_user.getUser_id() == manager.getUser_id() || auth_user.getRole().equals("admin")){
