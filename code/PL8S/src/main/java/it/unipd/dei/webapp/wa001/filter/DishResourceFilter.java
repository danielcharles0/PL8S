...
import it.unipd.dei.webapp.wa001.database.user.GetDishManagerDAO;
import it.unipd.dei.webapp.wa001.database.user.GetManagerDAO;
 ...
 */
public class DishResourceFilter implements Filter {

    /**
     * A LOGGER available for all the subclasses.
     */
    protected static final Logger LOGGER = LogManager.getLogger(DishResourceFilter.class, StringFormatterMessageFactory.INSTANCE);
    ...
    public void doFilter(final ServletRequest servletRequest, final ServletResponse servletResponse, final FilterChain chain) throws
        ...
        try {
            ...
            LOGGER.error("Unable to perform the restaurant resource filtering.", e);
            throw e;
        }
        ...
        try {
            ...
            if (req.getMethod().equals("GET") || req.getMethod().equals("PUT") || req.getMethod().equals("DELETE")) {

                int dish_id;

                try{
                    // get id of the dish to be modified from the URI
                    String pathinfo = req.getPathInfo();
                    String[] tokens = pathinfo.split("/");
                    dish_id = Integer.parseInt(tokens[2]);
                } catch (IndexOutOfBoundsException e) {
                    throw new Exception("No dish_id specified in the URL.");
                }

                // set dish_id as attribute to pass it to RRs
                req.setAttribute("dish_id", dish_id);

                manager = new GetDishManagerDAO(ds.getConnection(), dish_id).access().getOutputParam();

                // if there is a dish_id in the URL, it must be served by a restaurant managed by the user performing the request
                if (!authenticateUser(req, res, manager))
                    return;
            }
            ...
            if (req.getMethod().equals("PUT") || req.getMethod().equals("POST")) {
                ...
                dish = DishIngredient.fromJSON(req.getInputStream());

                // to overcome getInputStream()-read-only-once problem
                req.setAttribute("dish", dish);

                manager = new GetManagerDAO(ds.getConnection(), dish.getRestaurant()).access().getOutputParam();

                // if there is a body, the restaurant specified in it must belong to the user performing the request
                if (!authenticateUser(req, res, manager))
                    return;
            }
            ...
            LOGGER.error("Unable to retrieve the manager of the restaurant that serves this dish.", e);
            Message m = new Message("Unable to retrieve the manager of the restaurant that serves this dish.", ErrorCodes.INVALID_INPUT_PARAMETER, e.getMessage());
            res.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
            res.setContentType(JSON_UTF_8_MEDIA_TYPE);
            m.toJSON(res.getOutputStream());
        }   catch (Exception e) {
            LOGGER.error("Unable to perform the dish resource filtering.", e);
            Message m = new Message("Unable to perform the dish resource filtering.", ErrorCodes.INVALID_INPUT_PARAMETER, e.getMessage());
            res.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
            res.setContentType(JSON_UTF_8_MEDIA_TYPE);
            m.toJSON(res.getOutputStream());
        } finally {
     ...
     */
    private boolean authenticateUser(HttpServletRequest req, HttpServletResponse res, User manager) {
        ...
        try {
            ...
            if(auth_user != null) {
                ...
                // true if the authenticated user is the manager of the restaurant that serves this dish
                if(auth_user.getUser_id() == manager.getUser_id()){
