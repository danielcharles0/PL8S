...
/**
 * Checks for successful authentication to allow for accessing user resources.
 */
public class UserResourceFilter implements Filter {
     ...
     */
    protected static final Logger LOGGER = LogManager.getLogger(UserResourceFilter.class, StringFormatterMessageFactory.INSTANCE);
    ...
    public void doFilter(final ServletRequest servletRequest, final ServletResponse servletResponse, final FilterChain chain) throws
        ...
        try {
            ...
            LOGGER.error("Unable to perform the user resource filtering.", e);
            throw e;
        } finally {
    ...
    private boolean authenticateUser(HttpServletRequest req, HttpServletResponse res) {
        ...
        try {
            ...
            if (auth == null || auth.isBlank()) {
                ...
                sendAuthenticationChallenge(res, ErrorCodes.UNAUTHENTICATED_USER, "No authorization header sent.");
            ...
            if (!auth.toUpperCase().startsWith("BEARER ")) {
                ...
                sendAuthenticationChallenge(res, ErrorCodes.UNAUTHENTICATED_USER,"Bearer authentication is expected.");
            ...
            try {
                ...
                sendAuthenticationChallenge(res, ErrorCodes.UNAUTHENTICATED_USER, "Invalid JWT.");
                return false;
            }
            ...
            sendAuthenticationChallenge(res, ErrorCodes.UNAUTHORIZED_USER, "JWT expired.");
        } catch (Exception e) {
     ...
     */
    private void sendAuthenticationChallenge(HttpServletResponse res, String code, String details) throws IOException {

        try {
            res.setHeader("WWW-Authenticate", "Bearer realm=User");

            Message m = new Message("Cannot authenticate the user.", code, details);
